...
class Config:
    ...
    noise_area_frac: float = 0.0003   # drop connected components smaller than this
    ...
    cc_ref: float = 0.03              # largest-component area frac that maps to score 1.0
    ...
    net_change_min: float = 0.025     # min change vs pre-transition reference to keep event

    # --- transition state machine ---------------------------------------
    t_high: float = 0.30              # enter TRANSITIONING when score >= this (recall-first)
    ...
    dedup_hamming: int = 4            # merge events with pHash Hamming distance <= this
    dedup_min_gap_s: float = 0.5      # only dedup events within this temporal gap
